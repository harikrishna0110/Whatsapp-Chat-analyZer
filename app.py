    ...
    st.markdown(
        ...
        <div style="display: flex; justify-content: center; align-items: center; height: 100vh;">
            <div style="text-align: center; border: 2px solid {background_color}; padding: 20px; border-radius: 10px; background-color: {background_color}; color: {text_color}; box-shadow: 0px 0px 10px rgba(0,0,0,0.1);">
                <h2>Browse a zip file containing chat data and click on 'Show Analysis' to get the detailed report.</h2>
            </div>
        </div>
        """,
...
uploaded_file = st.sidebar.file_uploader("Choose a zip file")
if uploaded_file is not None:
    with zipfile.ZipFile(uploaded_file, "r") as zip_ref:
        zip_ref.extractall("temp_folder")

    txt_files = [file for file in os.listdir("temp_folder") if file.endswith(".txt")]
    if len(txt_files) > 0:
        file_path = os.path.join("temp_folder", txt_files[0])
        ...
        st.error("No text files found in the uploaded zip folder. Please make sure the zip file contains text files.")
else:
    display_initial_message()
